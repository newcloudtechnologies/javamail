...
import static javax.mail.util.BytesUtils.getBytesISO_8859_1;
...
public class InternetAddress extends Address implements Cloneable {
	...
	for (int i = 0; i < addresses.length; i++) {
		...
		sawNonAscii = true;
			if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.KITKAT) {
				as = new String(getBytesUTF8(as),
								StandardCharsets.ISO_8859_1);
			} else {
				try {
					as = new String(getBytesUTF8(as), "ISO-8859-1");
	...
	if (sawNonAscii)
		if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.KITKAT) {
			ret = new String(getBytesISO_8859_1(ret), StandardCharsets.UTF_8);
		} else {
			try {
				ret = new String(getBytesISO_8859_1(ret), "UTF-8");
			} catch (UnsupportedEncodingException e) {
				ret = new String(getBytesISO_8859_1(ret));
			}
